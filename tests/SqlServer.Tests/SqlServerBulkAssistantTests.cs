...
namespace SqlServer.Tests
{
    ...
    /// </summary>
    public class SqlServerBulkAssistantTests
    {
        private const string ConnectionString = "Server=sqlserver.local.com,1435;database=HysEMall_Promotion;uid=sa;pwd=yyy@123*;MultipleActiveResultSets=True;TrustServerCertificate=true";
        ...
        public void WriteToServer_WithShuffledColumns_DataMatchesExpected()
        {
            ...
            try
            {
                ...
                using var bulkAssistant = new SqlServerBulkAssistant(bulkCopy);
        ...
        private static void CreateTestTable(SqlConnection connection, string tableName)
        {
            var sql = $@"
        ...
        private static void CreateTestTableWithDefaults(SqlConnection connection, string tableName)
        {
            var sql = $@"
                CREATE TABLE [{tableName}] (
                    [id] INT NOT NULL,
        ...
        private static void CreateTestTableWithNullableColumns(SqlConnection connection, string tableName)
        {
            var sql = $@"
                CREATE TABLE [{tableName}] (
                    [id] INT NOT NULL,
        ...
        private static void DropTestTable(SqlConnection connection, string tableName)
        {
        ...
        /// 验证数据是否正确写入（完整列）。
        /// </summary>
        private static void VerifyData(SqlConnection connection, string tableName)
        {
        ...
        /// 验证部分列数据是否正确写入。
        /// </summary>
        private static void VerifyPartialData(SqlConnection connection, string tableName)
        {
            var sql = $"SELECT id, name, age, email, amount FROM [{tableName}] ORDER BY id";
            using var cmd = new SqlCommand(sql, connection);
            using var reader = cmd.ExecuteReader();
        ...
        /// </summary>
        private static void VerifyDataWithNullableColumns(SqlConnection connection, string tableName)
        {
