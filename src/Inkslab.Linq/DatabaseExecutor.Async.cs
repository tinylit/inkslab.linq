...
namespace Inkslab.Linq
{
    ...
    public partial class DatabaseExecutor
    {
        ...
        private class MultipleExecuteAsync : IAsyncMultipleExecutor
        {
            ...
            public async Task<T> ReadAsync<T>(CommandSql<T> commandSql)
            {
                if (_logger.IsEnabled(LogLevel.Debug))
                {
                    _logger.LogDebug(commandSql.ToString());
                ...
                try
                {
                    await using (var command = _connection.CreateCommand())
                    {
                        ...
                        await using (var reader = await command.ExecuteReaderAsync(behavior, _cancellationToken))
                        {
                            if (reader.HasRows)
                            {
                                var adaper = _adapters.GetOrAdd(
                                ...
                                if (await reader.ReadAsync(_cancellationToken))
                                {
                                    ...
                                    if (commandSql.RowStyle >= RowStyle.Single && await reader.ReadAsync(_cancellationToken))
                                    {
                                        ThrowMultipleRows(commandSql.RowStyle);
                                    }
            ...
            public IAsyncEnumerable<T> QueryAsync<T>(CommandSql commandSql)
            {
                ...
                return new AsyncEnumerableMultiple<T>(this, commandSql);
            }

            private async IAsyncEnumerator<T> ReadDeferredAsync<T>(CommandSql commandSql, CancellationToken cancellationToken)
            {
                CommandBehavior behavior = CommandBehavior.SequentialAccess | CommandBehavior.SingleResult;
            ...
            private class AsyncEnumerableMultiple<T> : IAsyncEnumerable<T>
            {
                private readonly MultipleExecuteAsync _executor;
                private readonly CommandSql _commandSql;

                public AsyncEnumerableMultiple(MultipleExecuteAsync executor, CommandSql commandSql)
                {
                    _executor = executor;
                    _commandSql = commandSql;
                }
                ...
                {
                    return _executor.ReadDeferredAsync<T>(_commandSql, cancellationToken);
                }
            }

            public async Task<IAsyncDbGridReader> QueryMultipleAsync(CommandSql commandSql)
            {
                if (_logger.IsEnabled(LogLevel.Debug))
                {
                    _logger.LogDebug(commandSql.ToString());
                ...
                try
                {
                    ...
                    reader = await command.ExecuteReaderAsync(behavior, _cancellationToken);
                    ...
                    return new AsyncDbGridReader(null, command, reader, commandSql, adaper);
                }
                catch
                {
                    if (reader != null)
        ...
        private class MultipleExecuteTimeoutAsync : IAsyncMultipleExecutor
        {
            ...
            private class AsyncEnumerableMultiple<T> : IAsyncEnumerable<T>
            {
                private readonly MultipleExecuteTimeoutAsync _executor;
                private readonly CommandSql _commandSql;

                public AsyncEnumerableMultiple(MultipleExecuteTimeoutAsync executor, CommandSql commandSql)
                {
                    _executor = executor;
                    _commandSql = commandSql;
                }
