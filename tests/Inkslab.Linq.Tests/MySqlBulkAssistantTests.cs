...
namespace Inkslab.Linq.Tests
{
    ...
    /// </summary>
    public class MySqlBulkAssistantTests
    {
        private const string ConnectionString = "server=mysql.local.com;uid=root;pwd=yyy@123*;database=framework;AllowLoadLocalInfile=true;Charset=utf8mb4;";
        ...
        public void WriteToServer_WithShuffledColumns_DataMatchesExpected()
        {
            ...
            try
            {
                ...
                using var bulkAssistant = new MySqlBulkAssistant(bulkCopy);
        ...
        private static void CreateTestTable(MySqlConnection connection, string tableName)
        {
            var sql = $@"
        ...
        private static void CreateTestTableWithDefaults(MySqlConnection connection, string tableName)
        {
            var sql = $@"
                CREATE TABLE `{tableName}` (
                    `id` INT NOT NULL,
        ...
        private static void CreateTestTableWithNullableColumns(MySqlConnection connection, string tableName)
        {
            var sql = $@"
                CREATE TABLE `{tableName}` (
                    `id` INT NOT NULL,
        ...
        private static void DropTestTable(MySqlConnection connection, string tableName)
        {
        ...
        /// 验证数据是否正确写入（完整列）。
        /// </summary>
        private static void VerifyData(MySqlConnection connection, string tableName)
        {
            ...
            Assert.Equal(1, reader.GetInt32("id"));
            Assert.Equal("Alice", reader.GetString("name"));
            Assert.Equal(25, reader.GetInt32("age"));
            Assert.Equal("alice@test.com", reader.GetString("email"));
            Assert.Equal(1000.50m, reader.GetDecimal("amount"));

            // 验证第二行
            Assert.True(reader.Read());
            Assert.Equal(2, reader.GetInt32("id"));
            Assert.Equal("Bob", reader.GetString("name"));
            Assert.Equal(30, reader.GetInt32("age"));
            Assert.Equal("bob@test.com", reader.GetString("email"));
            Assert.Equal(2000.75m, reader.GetDecimal("amount"));

            // 验证第三行
            Assert.True(reader.Read());
            Assert.Equal(3, reader.GetInt32("id"));
            Assert.Equal("Charlie", reader.GetString("name"));
            Assert.Equal(35, reader.GetInt32("age"));
            Assert.Equal("charlie@test.com", reader.GetString("email"));
            Assert.Equal(3000.00m, reader.GetDecimal("amount"));
        ...
        /// 验证部分列数据是否正确写入。
        /// </summary>
        private static void VerifyPartialData(MySqlConnection connection, string tableName)
        {
            var sql = $"SELECT id, name, age, email, amount FROM `{tableName}` ORDER BY id";
            using var cmd = new MySqlCommand(sql, connection);
            using var reader = cmd.ExecuteReader();
            ...
            Assert.Equal(1, reader.GetInt32("id"));
            Assert.Equal("Alice", reader.GetString("name"));
            Assert.Equal(0, reader.GetInt32("age")); // 默认值
            Assert.Equal("", reader.GetString("email")); // 默认值
            Assert.Equal(0.00m, reader.GetDecimal("amount")); // 默认值
        ...
        /// </summary>
        private static void VerifyDataWithNullableColumns(MySqlConnection connection, string tableName)
        {
